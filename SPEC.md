...
**Last updated:** 2026-05-02 (Session 4 — added decisions 10 and 11)
...
| 17 | Frontend | React + Vite + Tailwind + shadcn/ui frontend (chat pane, materials browser, multi-format preview via PDF.js / mammoth / marked). Bootstrapped with `anthropic-skills:web-artifacts-builder` patterns. Each major UI surface goes through an artifact-preview iteration loop in chat first (Milan approves the look, then it lifts into the real Vite project). Every surface ships with empty / loading / error states, keyboard navigation (`cmd+k` palette, `cmd+enter` send, `esc` to close), WCAG AA contrast, and screen-reader labels. First paint < 1s; chat stream latency < 200ms; PDF.js + mammoth lazy-loaded | |
