...
**Last updated:** 2026-05-02 (Session 5 — added decision 12: v0.2.0 repo separation)
...
10. **ROADMAP.md is canonical-only (added 2026-05-02 / Session 4):** ROADMAP.md lives ONLY in the canonical (private) repo. It is removed from `scripts/sync_to_public.py`'s ALLOWLIST and explicitly added to EXCLUDE_DST so `prune_excluded()` deletes any historical copy from the deploy-source repo on the next sync. ZIP_DROP_PATHS continues to drop `ROADMAP.md` from the user-facing zip as defense in depth. Rationale: ROADMAP carries SaaS-pivot speculation, repo-strategy notes, and other internal thinking that has no business sitting in the deploy-source repo even though that repo is now private. Asymmetry over the rest of the allowlist is deliberate — most docs benefit from being mirrored; ROADMAP doesn't.
11. **Local pre-push hook for branch protection (added 2026-05-02 / Session 4):** GitHub branch protection / rulesets are Pro-gated on free private repos (verified via API: HTTP 403 with "Upgrade to GitHub Pro"). Rather than upgrade the plan, the project ships a local pre-push hook at `references/git-hooks/pre-push` that refuses direct pushes to `main` with a clear error message pointing at the PR flow. `scripts/install-hooks.sh` is the idempotent installer; each clone runs it once. Bypass with `--no-verify` if intentional (rare; auto-sync publishes within ~30 seconds). Captured here so future Claude sessions know the protection mechanism is local-side, not server-side.

12. **v0.2.0 gets its own canonical + deploy-source pair (added 2026-05-02 / Session 5):** v0.1.x stays at `sharmingmilan/ultimate-job-assistant` + `sharmingmilan/ultimate-job-assistant-public` → `https://ultimatejobassist.netlify.app`. v0.2.0 gets `sharmingmilan/ultimate-job-assistant-v2` + `sharmingmilan/ultimate-job-assistant-v2-public` → `https://ultimatejobassist-v2.netlify.app`. Both new repos private (same posture as v0.1.x). Rationale: keeps v0.1.x site stable while v0.2.0 iterates; cleaner branch workflow on the new canonical (v0.2.0 work is `main` there, not a long-lived branch); inner-circle audience can have a separate URL without disrupting any existing v0.1.x users. Long-term shape (one site or two permanently) parked until Phase 17 ships and we can see what each site actually serves. See `V2_SETUP.md` for the operational steps and `scripts/sync_to_public_v2.py` for the v2-pair sync script.
...
| Phase | Subject | Deliverable | Status |
|---|---|---|---|
| 22.5 | Deprecation marking (target Session 10; no version bump) | Per ADR-002 D3. Add `host/frontend/README.md` flagging the directory as deprecated reference (HITL UX patterns demo for v0.4.0). Docstring headers on `host/uja_host/main.py`, `api/chat.py`, `api/conversations.py` marking deprecated. Demote `host/uja_host/keystore.py` + `api/auth.py` (module docstrings explain "kept for future credential-store / headless export, not load-bearing in v0.2.x"). One atomic commit, PR-driven merge. Cheap test of the Dispatch-session pattern before Phase 23 commits to a longer session. | committed |
| 23 | MCP server scaffold (target v0.2.1, Session 11) | `host/uja_mcp/server.py` (JSON-RPC over stdio per the `mcp-builder` skill's Python guidance) + `host/uja_mcp/tools/{files,skills,hitl}.py` (thin facades over the surviving `host/uja_host/` modules). Register the ADR-002 D1 tool surface (file + skill + HITL + read_workspace_metadata). Pin `mcp` in `host/requirements.txt`. Re-point Phase 15/16/17.5 tests at MCP tool functions. New `host/tests/test_mcp_server.py` for protocol-level tests. New `start-uja-mcp.sh` / `start-uja-mcp.bat` launcher + `references/cowork-mcp-config-snippet.json`. Tag `v0.2.1` end of session. (`export_application` deferred to Phase 24.) | ✅ shipped Session 11 (merge `e634252`, tag `v0.2.1`) |
